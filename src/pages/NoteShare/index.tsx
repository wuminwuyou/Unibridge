...
import { useNoteFeedData } from '../../api/feed/useNoteFeedData'
...
import { experienceShareTags } from './experienceSharePageData'
import './style.css'
 ...
 * 功能：渲染"经验分享"频道页面，笔记列表来自 GET /feed/notes?noteType=IMAGE_TEXT。
 * 实现方法：
 * - 复用 TopNavbar 渲染全局顶部导航
 * - 上方渲染学科大类标签筛选栏，便于后续接入过滤逻辑
 * - 下方渲染笔记专区，右侧外置设置与"换一换"操作按钮
 * - 点击「换一换」调用 GET /feed/notes/shuffle
 * 输入：无
 * 输出：
...
function ExperienceSharePage() {
  ...
  const renderedGridNoteCards = useMemo(() => {
    return notes.map((note) => <GridNoteCard key={note.uid ?? note.title} note={note} />)
  }, [notes])
    ...
    <div className="experience-share-page">
      ...
      <main className="experience-share-main">
        ...
        <section className="experience-note-section" aria-label="经验分享笔记专区">
          <div
            className={`experience-note-panel ${isShuffling ? 'experience-note-panel--shuffling' : ''}`.trim()}
          >
            {loadState === 'loading' ? (
              <div className="experience-note-feed-status">
                <LoadingSpinner size={32} label="正在加载推荐笔记…" />
              </div>
            ) : null}

            {loadState === 'error' ? (
              <p className="experience-note-feed-status experience-note-feed-status--error" role="alert">
                {errorMessage ?? '加载推荐笔记失败，请稍后重试'}
              </p>
            ) : null}

            {loadState === 'ready' && notes.length === 0 ? (
              <p className="experience-note-feed-status">暂无推荐笔记</p>
            ) : null}
            ...
            {isShuffling ? (
              <div className="experience-note-shuffle-overlay" aria-hidden="true">
                <LoadingSpinner size={28} label="换一换中…" />
              </div>
            ) : null}
          </div>
