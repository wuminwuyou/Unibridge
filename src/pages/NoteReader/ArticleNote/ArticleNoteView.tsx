...
import {
  ...
  useMarkdownReaderId,
} from './components/MarkdownReader'
import {
...
import './ArticleNoteView.css'

// 01）图文笔记阅读视图 Props（ArticleNoteViewProps）
interface ArticleNoteViewProps {
...
// 05）图文笔记阅读视图（ArticleNoteView）
export function ArticleNoteView({ note, isEditorialFlow = false }: ArticleNoteViewProps) {
      ...
      <div className="w-[clamp(1380px,80vw,1500px)] mx-auto px-4 py-6">
        <div className="grid grid-cols-12 gap-8 items-start w-full">
          ...
          {showMarkdownCatalog ? (
            <aside
              ref={tocRef}
              className={`col-span-3 w-[clamp(280px,22vw,340px)] self-start article-note-reader__toc-aside ${tocSticky ? `sticky ${stickyTopClass}` : ''
                }`}
              aria-label="文章目录"
            >
              <MarkdownMdCatalogPanel
          ...
          <aside className={`col-span-3 w-[clamp(280px,22vw,340px)] sticky ${stickyTopClass} self-start justify-self-end`}>
            <div className="flex flex-col space-y-6">
              ...
              {/* 卡片 A：作者信息 */}
              <div className="article-note-reader__sidebar-card p-4">
                <h3 className="article-note-reader__sidebar-card-title mb-3">
                  <User size={14} aria-hidden="true" />
                  作者
                </h3>
                <div className="article-note-reader__author-block">
                  {authorProfilePath ? (
                    <>
                      <Link to={authorProfilePath} className="article-note-reader__author-link">
                        {note.author.avatarUrl ? (
                          <img className="article-note-reader__author-avatar" src={note.author.avatarUrl} alt={note.author.name} />
                        ) : (
                          <span className="article-note-reader__author-avatar article-note-reader__author-avatar--fallback">{authorInitial}</span>
                        )}
                        <div className="article-note-reader__author-text">
                          <p className="article-note-reader__author-name">{note.author.name}</p>
                          <p className="article-note-reader__author-org">{organizationText}</p>
                        </div>
                      </Link>
                      <button type="button" className="article-note-reader__follow-btn" aria-label="关注作者">
                        关注
                      </button>
                    </>
                  ) : (
                    <div className="article-note-reader__author-row">
                      {note.author.avatarUrl ? (
                        <img className="article-note-reader__author-avatar" src={note.author.avatarUrl} alt={note.author.name} />
                      ) : (
                        <span className="article-note-reader__author-avatar article-note-reader__author-avatar--fallback">{authorInitial}</span>
                ...
                <div className="article-note-reader__stats-row">
                  <span className="article-note-reader__stats-item"><Eye size={12} aria-hidden="true" />{note.views} 浏览</span>
                  <span className="article-note-reader__stats-item"><Heart size={12} aria-hidden="true" />{note.favorites} 收藏</span>
                  <span className="article-note-reader__stats-item"><ThumbsUp size={12} aria-hidden="true" />{note.comments} 评论</span>
                </div>
              </div>

              {/* 卡片 B：父笔记快速入口 */}
              {note.parentNote ? (
                <div className="article-note-reader__sidebar-card">
                  <h3 className="article-note-reader__parent-card-header">
              ...
              {note.uid ? (
                <div
                  ref={editorCardRef}
                  className="article-note-reader__editor-card article-note-reader__sidebar-card flex flex-col"
                  style={editorStyle}
                >
                  {/* 折叠头部按钮 */}
                  <button
                  ...
                  >
                    <h3 className="article-note-reader__sidebar-card-title">
                      <BookOpen size={14} aria-hidden="true" />
                      学习笔记
                    </h3>
                    {isEditorExpanded ? (
                  ...
                  {isEditorExpanded ? (
                    <div className="article-note-reader__editor-card-body">
                      <NoteQuickMdEditor
