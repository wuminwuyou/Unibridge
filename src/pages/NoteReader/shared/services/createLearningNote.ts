...
import type { LearningNoteParentPayload } from '../types/learningNoteParentPayload'
 ...
 * 功能：将父笔记上下文与学习笔记 Markdown 组装为 POST /notes 创建请求体（图文子笔记）。
 * 实现方法：
 * - 通过 extractAutoSummaryFromMarkdown 自动生成摘要
 * - contentType 固定为「图文」，tags 继承父笔记，visibility 默认 PRIVATE
 ...
 * 实现方法：
 * - 校验表单与父笔记 uid
 * - 根据学习笔记自身的 title 用 generateAndUploadNoteCover 生成并上传封面
 * - buildCreateLearningNoteRequest 组装请求体
 * - createNote 写入服务端
 * 输入：
 * - note：父笔记载荷
