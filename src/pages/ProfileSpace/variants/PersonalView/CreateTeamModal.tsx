...
import { TeamProfileApiError, createStudentTeam } from '../../../../api/teamProfile'
import { getUserVerifiedPreview, UserApiError } from '../../../../api/users'
import { isUserResourceUid } from '../../../../api/resourceUid'
import type { UserVerifiedPreviewDto } from '../../../../api/users/types'
import { buildTeamSpacePath } from '../TeamView/teamTabRouting'

// 01）创建团队弹窗 Props（CreateTeamModalProps）
interface CreateTeamModalProps {
  open: boolean
  onClose: () => void
}

// 02）创建团队弹窗（CreateTeamModal）
/**
 * 函数名：CreateTeamModal
 * 功能：提供创建学生团队的表单弹窗。
 * 输入：
...
export function CreateTeamModal({ open, onClose }: CreateTeamModalProps) {
  ...
  // 初始成员 UID 输入 → 300ms debounce → getUserVerifiedPreview
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      // 创建者自动成为 LEADER；仅当填了成员 UID 时才额外传递
  ...
  const entering = open
  const bc = entering ? 'create-team-modal-backdrop create-team-modal-backdrop--enter' : 'create-team-modal-backdrop create-team-modal-backdrop--exit'
  const cc = entering ? 'create-team-modal-card create-team-modal-card--enter' : 'create-team-modal-card create-team-modal-card--exit'
      ...
      <div className={cc}>
        ...
        <form className="create-team-modal__form" onSubmit={(e) => { e.preventDefault(); void handleSubmit() }}>
          <div className="create-team-modal__field">
            <label htmlFor="create-team-name" className="create-team-modal__label">团队名称 <span className="create-team-modal__required">*</span></label>
            <input ref={nameInputRef} id="create-team-name" type="text" className="create-team-modal__input"
              value={teamName} onChange={(e) => { setTeamName(e.target.value); if (errorMessage) setErrorMessage(null) }}
          ...
          <div className="create-team-modal__field">
            ...
            <textarea id="create-team-desc" className="create-team-modal__textarea" value={teamDescription}
              onChange={(e) => setTeamDescription(e.target.value)} placeholder="一句话介绍团队方向…" rows={3} maxLength={120} disabled={isSubmitting} />
          ...
          <div className="create-team-modal__field">
            ...
            {memberPreview ? (
              <div className="create-team-modal__leader-preview">
                <img className="create-team-modal__leader-avatar"
              ...
              <input id="create-team-member" type="text" className="create-team-modal__input"
                value={memberUid} onChange={(e) => handleMemberUidChange(e.target.value)}
          ...
          <div className="create-team-modal__actions">
            <button type="button" className="create-team-modal__btn create-team-modal__btn--cancel" onClick={handleClose} disabled={isSubmitting}>取消</button>
            <button type="submit" className="create-team-modal__btn create-team-modal__btn--submit" disabled={isSubmitting || !teamName.trim()}>
