...
import type { TeamMemberRole } from '../../components/types'
import {
  buildMemberMetaSegments,
  shouldShowMemberMetaSeparator,
} from '../../components/MemberCard/memberCardUtils'
import type { ProfileMemberItem } from '../../components/types'
import { useMembersManageForm, type EditableMemberItem } from './useMembersManageForm'
import './ManageMembersForm.css'

// 01）管理成员表单 Props（ManageMembersFormProps）
...
// 03）成员身份只读展示（ManageMemberIdentityCell）
interface ManageMemberIdentityCellProps {
  member: EditableMemberItem
}
      ...
      {segments.map((segment, index) => (
        <span key={`${segment.kind}-${segment.label}`} className="team-manage-members-form__identity-item">
          {shouldShowMemberMetaSeparator(segments, index) ? (
            <span className="team-manage-members-form__identity-separator" aria-hidden="true">
              ·
            </span>
          ) : null}
          {segment.kind === 'owner' ? (
...
// 04）管理成员表单（ManageMembersForm）
/**
 * 函数名：ManageMembersForm
 * 功能：提供空间成员管理表单；身份（role）只读，团队定位（career）可编辑，非负责人可切换 isAdmin 协助管理。
 * 输入：
 * - teamUid：团队 uid
 * - isLabSpace / isLoggedIn / isViewerTeamMember：姓名展示上下文
 * - members：当前成员列表
 * - onCancel：取消并返回成员列表
 * - onSaved：保存成功后返回成员列表
 * 输出：
                ...
                {form.members.map((member) => {
                  ...
                  return (
                    <tr key={member.uid}>
                      ...
                      <td>{member.uid}</td>
                      <td>
                        <ManageMemberIdentityCell member={member} />
                      </td>
                      <td>
                        <input
