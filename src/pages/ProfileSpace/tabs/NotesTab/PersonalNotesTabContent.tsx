...
import { usePersonalNotesTabData } from './usePersonalNotesTabData'
import './PersonalNotesTabContent.css'

// 01）笔记筛选项类型（NoteFilterType）
type NoteFilterType = '全部' | '图文' | '视频'

// 02）笔记筛选项列表（noteFilters）
const noteFilters: NoteFilterType[] = ['全部', '图文', '视频']

// 03）笔记卡片布局模式（NoteCardLayoutMode）
type NoteCardLayoutMode = 'grid' | 'row'

// 04）布局缓存键（NOTE_CARD_LAYOUT_STORAGE_KEY）
const NOTE_CARD_LAYOUT_STORAGE_KEY = 'unibridge.profile.notes.cardLayout'

// 05）解析初始布局模式（resolveInitialNoteCardLayoutMode）
