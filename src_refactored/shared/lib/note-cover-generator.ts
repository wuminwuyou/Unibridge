...
// 04）自适应最佳字号（measureBestFontSize）
...
// 06）将 Data URL 转为 Blob（dataUrlToBlob）
...
// 07）从 Data URL 创建 File 对象（dataUrlToFile）
 ...
 * - title：笔记标题
 * 输出：
 * - 返回值：base64 图片 Data URL（image/jpeg）
 */
...
// 09）生成封面 File 对象（autoGenerateNoteCoverFile）
/**
 * 函数名：autoGenerateNoteCoverFile
 * 功能：调用 autoGenerateNoteCoverDataUrl 生成 Canvas 封面 → 转为 File 对象，供上传使用。
 * 输入：
 * - title：笔记标题
 * 输出：
 * - 返回值：File（image/jpeg）
 */
export function autoGenerateNoteCoverFile(title: string): File {
  const dataUrl = autoGenerateNoteCoverDataUrl(title)
  return dataUrlToFile(dataUrl, `note-cover-${Date.now()}.jpg`)
}
