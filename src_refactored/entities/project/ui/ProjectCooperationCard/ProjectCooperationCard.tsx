...
// 职责：展示预算、周期、团队、截止四项信息
// 禁止：navigate / API / feature import
import { CircleDollarSign, Clock3, Users, Calendar } from 'lucide-react'
 ...
 * 功能：纯展示项目合作信息：预算 / 周期 / 团队 / 截止。
 * 输入：
 * - amount / duration / teamSize / deadline：四项文案
 * - icons：可选图标插槽
 * 输出：
 * - 返回值：React 节点
 */
  ...
  return (
    <section className={styles.card}>
      ...
      <ul className={styles.facts}>
        ...
        <li className={styles.factItem}>
          {icons?.deadline ?? <Calendar className="h-4 w-4" aria-hidden="true" />}
          <span>截止</span>
          <strong className={styles.factValue}>{deadline}</strong>
        </li>
      </ul>
    </section>
  )
