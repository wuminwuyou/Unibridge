...
import type { UserPublicPreviewDto, UserVerifiedPreviewDto } from '../model/types'
...
export async function getUserPublicPreview(uid: UserResourceUid): Promise<UserPublicPreviewDto> {
  ...
  try {
    ...
    const realName = typeof realNameRaw === 'string' && realNameRaw.trim().length > 0 ? realNameRaw.trim() : null
    return {
    ...
    if (error instanceof HttpApiError) throw new UserApiError(error.code, error.message)
    throw error
  }
}
  ...
  try {
    ...
    return (users as Array<Record<string, unknown>>).map((raw) => {
      ...
      return {
        ...
        realName: typeof realNameRaw === 'string' && realNameRaw.trim().length > 0 ? realNameRaw.trim() : null,
