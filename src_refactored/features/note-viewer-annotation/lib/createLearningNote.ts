import { NotesApiError, createNote, uploadNoteCover } from '../../../entities/note/api/noteApi'
import type { NotePublishAction, UpsertNoteRequest } from '../../../entities/note/model/types'
import { isNoteResourceUid } from '../../../shared/api/resourceUid'
import type { NoteResourceUid } from '../../../shared/api/resourceUid'
import { extractAutoSummaryFromMarkdown } from '../../../shared/lib/publishSummary'
 ...
 * 功能：将父笔记上下文与学习笔记 Markdown 组装为 POST /notes 创建请求体。
 * 输入：
 * - note：父笔记载荷
 ...
 * 功能：创建一篇与父笔记关联的图文学习笔记。
 * 实现方法：
 * - 校验表单与父笔记 uid
 * - 自动生成封面并上传
 * - 组装请求体 → createNote 写入服务端
 * 输入：
 * - note：父笔记载荷
 * - noteUid：笔记对外 uid（兼容保留）
 * - markdown：学习笔记 Markdown
 * - publishAction：DRAFT 或 PUBLISH
 * 输出：
 * - 返回值：笔记 uid
...
export async function createLearningNote(options: {
  ...
  const coverFile = autoGenerateNoteCoverFile(learningNoteTitle)
  const uploaded = await uploadNoteCover(coverFile, 'auto', coverFile.name)
  const coverUrl = uploaded.coverUrl?.trim()
  if (!coverUrl) {
    throw new NotesApiError(500, '封面生成后上传未返回封面地址')
  }
